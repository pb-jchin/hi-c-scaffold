...
def main():
    ...
    with open("new_links_sorted",'r') as f:
        for row in f:
            ...
            c1 = v1.split(':')[0]
            c2 = v2.split(':')[0] 
            if c1 in to_merge and c2 in to_merge:
                if contig_length[c1] > 50000 and contig_length[c2] > 50000:
                    if v1 not in existing_nodes and v2 not in existing_nodes:
                        if score > 300:
                            G_small.add_edge(v1,v2,score=score)
                            contigs.add(c1)
                            contigs.add(c2)
                            existing_nodes.add(v1)
                            existing_nodes.add(v2)
    ...
    for each in contigs:
        G_small.add_edge(each+':B',each+':E',score=0)

    for subg in nx.connected_component_subgraphs(G_small):
        p0 = []
        ...
        if len(p0) == 2:
            path = nx.shortest_path(G_small,p0[0],p0[1])
            if len(path) >= 10:
                backbone_paths[path_id] = path
                path_id += 1
    ...
    for key in backbone_paths:
        if len(backbone_paths[key]) >= 4:
            ...
            recs.append(rec)
            c_id += 1
    SeqIO.write(recs,'scaffolds_new_5.0.fasta','fasta')
